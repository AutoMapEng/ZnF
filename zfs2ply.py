#!/usr/bin/env python3
"""
zfs2ply.py - Convert a Z+F PROFILER (.zfs) scan file to a PLY point cloud.

Usage:
    python3 zfs2ply.py INPUT.zfs [-o OUTPUT.ply] [options]

Output PLY vertex fields:  x, y, z (float32), intensity (float32), timestamp (float64)
...
def write_ply(path, x, y, z, inten, tstamp, ascii_mode, meta):
    npts = x.size
    header = (
        "ply\n"
        f"format {'ascii' if ascii_mode else 'binary_little_endian'} 1.0\n"
        "comment Generated by zfs2ply.py from a Z+F PROFILER .zfs scan\n"
        ...
        with open(path, "w") as f:
            ...
            for i in range(npts):
                f.write(f"{x[i]:.6f} {y[i]:.6f} {z[i]:.6f} "
                        f"{inten[i]:.1f} {tstamp[i]:.3f}\n")
    else:
        rec = np.empty(npts, dtype=[("x", "<f4"), ("y", "<f4"), ("z", "<f4"),
                                    ("intensity", "<f4"), ("timestamp", "<f8")])
        rec["x"], rec["y"], rec["z"] = x, y, z
        rec["intensity"], rec["timestamp"] = inten, tstamp
...
def main():
    ap = argparse.ArgumentParser(
        description="Convert a Z+F .zfs scan to a PLY point cloud "
                    "(x, y, z, intensity, timestamp).")
    ap.add_argument("input", help="input .zfs file")
    ap.add_argument("-o", "--output", help="output .ply (default: input name + .ply)")
    ...
    ap.add_argument("--ascii", action="store_true",
                    help="write ASCII PLY instead of binary")
    args = ap.parse_args()

    out_path = args.output or (os.path.splitext(args.input)[0] + ".ply")
    ...
    write_ply(out_path, x, y, z, inten, tstamp, args.ascii, meta)

    print(f"{args.input}: {n_prof} profiles x {points} points x {channels} channels")
    print(f"timestamp span: {ts[0]:.0f} .. {ts[-1]:.0f} "
          f"({(ts[-1] - ts[0]) / 1000.0:.2f} s @ ~{n_prof / max((ts[-1]-ts[0])/1000.0, 1e-9):.0f} profiles/s)")
    print(f"wrote {out_path}: {x.size:,} points "
          f"({'ascii' if args.ascii else 'binary_little_endian'})")
