...
namespace zfsfilter
{
    ...
    /** \class MixedPixelFilterEnhanced
    ...
    class FILTERDLL_API MixedPixelFilterEnhanced: virtual public TileFilter, public MultithreadingBase // virtuelle Vererbung erfordert explizite Initialisierung von Basisklassen in höheren Vererbungsstufen
    {
    public:
        // ---------------------------------------------------------------------------------------------------------------------
...
//        void prepare_filteringZFScan();
        ...
        void initializeMT();
        static void initializeThreadWorker( MixedPixelFilterEnhanced* const pMixedPixelFilterEnhanced, MPWStatistics<uint16_t>* const pMPWStatistics, const int32_t& line_start, const int32_t& line_end );
        ...
        static void updateOutputMaskThreadWorker( MixedPixelFilterEnhanced* const  pMixedPixelFilterEnhanced,
                                                  const zfs::math::BoundingBox                 & BoundingBoxSource,
                                                  const zfs::math::BoundingBox                 & BoundingBoxTarget,
    ...
    private:
        ...
        void                   transfer_data_to_file_ZFScan();
    ...
    public:
        ...
        // -- constructors
        MixedPixelFilterEnhanced();

        MixedPixelFilterEnhanced( const std::wstring&             a_FilePath,
                                  const int32_t&                  a_PixelStart,
                                  const int32_t&                  a_PixelEnd,
                                  const int32_t&                  a_LineStart,
                                  const int32_t&                  a_LineEnd,
                                  const FilterStrengthOptionType& a_FilterStrengthOption,
                                  const ProcessingDirOptionsType& a_FilterDirectionOption,
                                  const float&                    a_ProfileProgressFeed,
                                  const bool&                     a_bNormalAnalysis,
                                  const bool&                     a_bKalmanAnalysis,
                                  const bool&                     a_bEdgeAnalysis,
                                  const bool&                     a_bSmooth,
                                  const bool&                     a_bMaskToOriginal,
        ...
        MixedPixelFilterEnhanced( const std::wstring&                    a_FilePath,
                                  ...
                                  const int32_t&                         a_LineStart,
                                  const int32_t&                         a_LineEnd,
                                  const double&                          a_FilterStrength,
                                  const ProcessingDirOptionsType&        a_FilterDirectionOption,
                                  RollingBuffer::cb_GetProfilerXYZLine_f a_GetProfilerXYZLineFunction,
                                  const bool&                            a_bNormalAnalysis,
                                  const bool&                            a_bKalmanAnalysis,
                                  const bool&                            a_bEdgeAnalysis,
                                  const bool&                            a_bSmooth,
        ...
        /*
        MixedPixelFilterEnhanced( ZFScan* const                   a_ZFScan,
                                  const int32_t&                  a_PixelStart,
                                  const int32_t&                  a_PixelEnd,
                                  const int32_t&                  a_LineStart,
                                  const int32_t&                  a_LineEnd,
        ...
        MixedPixelFilterEnhanced( ZFScan* const                          a_ZFScan,
                                  ...
                                  const int32_t&                         a_LineStart,
                                  const int32_t&                         a_LineEnd,
                                  const double&                          a_FilterStrengthValue,
                                  RollingBuffer::cb_GetProfilerXYZLine_f a_GetProfilerXYZLineFunction,
                                  const bool&                            a_bNormalAnalysis,
                                  const bool&                            a_bKalmanAnalysis,
                                  const bool&                            a_bEdgeAnalysis,
        ...
        // -- alternative use using parameter object
    ...
    // helper functions
    // ---------------------------------------------------------------------------------------------------------------
    // application in project contex
//    FILTERDLL_API ZFScan*  MixedPixelFilterEnhancedInPrjContext( zfs::zfscan_ptr& apZfsPtr, FilterResultItem& aItem, const ParamsMixedpixelNew& aParams, const ZFRect& aRCSel );
    // ---------------------------------------------------------------------------------------------------------------
} // namespace zfsfilter
