...
namespace kdt
{
    ...
    */
    ...
    public:
        ...
        */
        virtual ~KDTree() { clear(); }

        PointT at(int idx) const { return points_[idx]; }
        ...
        */
        ...
        void build(std::vector<PointT> points)
        {
            clear();
        ...
        */
        int nnSearch(PointT query, double* minDist = nullptr) const
        {
            int guess=0;
        ...
        */
        std::vector<int> knnSearch(PointT query, int k) const
        {
            KnnQueue queue(k);
        ...
        */
        std::vector<int> radiusSearch(PointT query, double radius) const
        {
        ...
        /** @brief Searches neighbors within radius.
        */
        std::vector<int> boxSearch(PointT query, std::vector<double>& c) const
        {
            std::vector<int> indices;
            boxSearchRecursive(query, root_, indices, c);
            return indices;
        }

        /** @brief Searches neighbors within radius.
        */
        std::vector<PointT> radiusSearchPt(PointT query, double radius ) const
        {
            std::vector<int> indices;
            radiusSearchRecursive(query, root_, indices, radius);

            std::vector<PointT> points(indices.size());
            
            int pi = 0;
            for(auto &i : indices)
            {
                points[pi] = points_[i]; ++pi;
            }
    ...
    private:
        ...
        */
        ...
        {
            double dist = 0;
            for (uint32_t i = 0; i < PointT::DIM; i++)
        ...
        */
        ...
        /** @brief Searches neighbors within radius, returns point indices.
        */
        ...
        static bool boxCoordCond( double pi, double qi, double ci )
        {
            return fabs(pi - qi) <= ci;
        }

        static bool boxCond( const PointT& p, const PointT& q, std::vector<double>& c )
        {
            bool cond = true;
            for (int i = 0; i < PointT::DIM; i++)
                cond &= boxCoordCond( p[i], q[i], c[i] );
            return cond;
        }

        /** @brief Searches neighbors within radius, returns point indices.
        */
        void boxSearchRecursive(const PointT& query, const Node* node, std::vector<int>& indices, std::vector<double>& c) const
        {
            if (node == nullptr)
                return;
            ...
            if ( boxCond(query,train,c) )
                indices.push_back(node->idx);

            const int axis = node->axis;
            const int dir = query[axis] < train[axis] ? 0 : 1;
            boxSearchRecursive(query, node->next[dir], indices, c);

            if ( boxCoordCond( query[axis], train[axis], c[axis] ) )
                boxSearchRecursive(query, node->next[!dir], indices, c);
        }

        /** @brief Searches neighbors within radius, returns points.
        */
        void radiusSearchRecursivePt( const PointT& query, const Node* node, std::vector<PointT>& points, double radius ) const
        {
            if (node == nullptr)
                return;
            ...
            const double dist = distance(query, train);
            if (dist < radius)
                points.push_back(train);

            const int axis = node->axis;
            const int dir = query[axis] < train[axis] ? 0 : 1;
            radiusSearchRecursivePt(query, node->next[dir], points, radius);
