...
namespace zf
{
    ...
    */
    class ZFCORE_API Argument
    {
    public:
        Argument() {}
        Argument(int argc, char* argv[]);
        Argument(int argc, wchar_t* argv[]);
        Argument(const std::string&); // use parseAndAddItems
        Argument(const std::wstring&); // use parseAndAddItems

        typedef std::map<std::string, std::string> arg_map; // utf8
        ...
        void parseAndAddItems(const std::wstring&);
